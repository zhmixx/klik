...
levelbar.set(0)
level_label = customtkinter.CTkLabel(stats_row, text=f'level: {level}', width=40, height=28, fg_color='transparent')
level_label.pack(side="left", padx=10, pady=0)
expamount = customtkinter.CTkLabel(stats_row, text=f"exp: {exp}/{exp_to_next}", width=40, height=28, fg_color='transparent')
...
def gain_exp(amount: int):
    ...
    while exp >= exp_to_next:
        ...
        kliker.configure(fg_color=normal_colors[index], hover_color=darkened_colors[index])
    expamount.configure(text=f"exp: {exp}/{exp_to_next}")
...
kliker = customtkinter.CTkButton(click_row, text='', width=120, height=120, command=klik, image=klikimg, fg_color='white', hover_color='gray')
kliker.pack(expand=True)

klikamount = customtkinter.CTkLabel(click_row, text=f"kliks: {kliks}", width=40, height=28, fg_color='transparent')
klikamount.pack(side='top')

def open_shop():
  pg1.pack_forget() # hide main page
  pg2.pack(fill='both', expand=True) # ... and show the shop

shopbutton = customtkinter.CTkButton(btn_row, text='shop', width=140, height=28, command=open_shop)
...
savebutton = customtkinter.CTkButton(btn_row, text='save', width=140, height=28, command=lambda: save_variables(get_savevars()))
savebutton.pack(pady=5, padx=5)

pg2 = customtkinter.CTkFrame(app) # shop

def close_shop():
  pg2.pack_forget()
  pg1.pack(fill='both', expand=True)

close_shop = customtkinter.CTkButton(pg2, text='<< close shop', width=100, height=28, command=close_shop)
